  ...
  constructor(public appInfo: AppInfoService) {
    ...
    loadMessages({
      'fr': {
        ...
        "custom_please_note_source_directory": "Veuillez noter que ces scripts ciblent le répertoire /source du dossier dans lequel ils se trouvent (à l'exception de tei_for_txm_to_uima.py qui cible /TEI), si jamais vous souhaitez les utiliser dans un autre contexte.",
      ...
      'en': {
        ...
        "custom_please_note_source_directory": "Please note that these scripts target the /source directory in the directory they're currently in (with the exception of tei_for_txm_to_uima.py which targets /TEI), if you wish to use them in a different context.",
        ...
        "custom_uima_to_tei_txm": "The uima_to_tei_for_txm.py script allows to convert to a TXM-compatible format (tested with version 0.8.2). Please note that this requires to have either launched stanza_parse.py or exported the corpus from INCEpTION, otherwise there will be no tokenisation and TXM won't accept it. All annotations and their attributes should be available in CQP requests.",
        "custom_tei_txm_to_uima": "The tei_for_txm_to_uima.py scripts allows to convert back an XML-TXM export to the UIMA format. This keeps all the tags initially present in the UIMA files.",
    ...
    */
    ...
    if (event.value && event.value[0]) {
      ...
      reader.onload = () => {
        ...
        var reader_result = reader.result.toString().replace((new RegExp("(<[^<>]+\\s)(sofa|begin|end|type)(=\"[^\"]*\"[^<>]*>)","gmi")),"$1$2_$3");
