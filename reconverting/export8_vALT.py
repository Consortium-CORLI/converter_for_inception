...
if err:
    ...
    for i in range(len(current_dir_list)):
        if len(regex_zip.findall(current_dir_list[i])) > 0 and os.path.isfile(current_dir_list[i]):
            ...
            for j in range(len(source_files_list)):
                ...
                if index < 0:
                    ...
                    for k in range(len(sub_zip_files)):
                        if sub_zip_files[k].endswith('.xmi'):
                            #MOD
                            r = re.findall('(.*)\\.xmi$',sub_zip_files[k])
                            print(r)
                            if r[0] not in annotators_dict.keys():
                                annotators_dict[r[0]] = references_up_to_now[:]
                            ...
                            annotators_dict[r[0]].append(local_zip_path + '/' + sub_zip_files[k])
                    ...
                    for m in range(len(sub_zip_list_dir)):
                        if os.path.isdir(sub_zip_path + '/' + sub_zip_list_dir[m]) or (not sub_zip_list_dir[m].endswith('.zip')):
                            continue
                        # local_zip_path = sub_zip_path + '/__local_zip'
                        local_zip_path = sub_zip_path + '/__local_zip' + str(m)
                        # with zipfile.ZipFile(sub_zip_path + '/' + sub_zip_list_dir[0],'r') as current_zip_file:
                        with zipfile.ZipFile(sub_zip_path + '/' + sub_zip_list_dir[m],'r') as current_zip_file:
                            current_zip_file.extractall(local_zip_path)
                        sub_zip_files = os.listdir(local_zip_path)
                        found = False
                        for k in range(len(sub_zip_files)):
                ...
                #MOD
                references_up_to_now.append(source_files_path + '/' + source_files_list[j])
                len_references_up_to_now = len(references_up_to_now)
                for k in annotators_dict.keys():
                    if len(annotators_dict[k]) < len_references_up_to_now:
                    # if len(annotators_dict[k]['references_dict'][re.sub('\\.[^\\.]+$','',source_files_list[j])]) < len_references_up_to_now:
                        annotators_dict[k].append(references_up_to_now[-1])
                        # annotators_dict[k]['references_dict'][re.sub('\\.[^\\.]+$','',source_files_list[j])]['xml_uri'] = references_up_to_now[-1]
                del(len_references_up_to_now)
    ...
    # for _i in annotators_dict.keys():
    #     print(_i,annotators_dict[_i][:10])
    ...
    for a in references_dict.keys():
        ...
        print('Building tree:',references_dict[a]['xml_uri'],end='\r')
        # print('Building tree:',references_dict[a]['xml_uri'])
        #MOD
        """
        ...
        """
        #MOD
        # local_pattern = '[\\/\\\\]' + re.findall('[^/]*_doc[0-9]+\\.xml',references_dict[a]['xml_uri'])[0] + '[\\/\\\\].*\\.xmi'
        local_pattern = '[\\/\\\\]' + re.findall('[^/]*_doc[0-9]+\\.xml',references_dict[a]['xml_uri'])[0]
        # print(local_pattern,end='\r')
        inner_content = {}
        for b in annotators_dict.keys():
            local_index = -1
            for c in range(len(annotators_dict[b])):
                # if len(re.findall('[\\/\\\\]' + str(references_dict[a]['xml_uri']) + '[\\/\\\\].*\\.xmi',annotators_dict[b][c])) > 0:
                # local_pattern = '[\\/\\\\]' + str(references_dict[a]['xml_uri']) + '[\\/\\\\].*\\.xmi'
                #MOVED
                # local_pattern = '[\\/\\\\]' + re.findall('[^/]*_doc[0-9]+\\.xml',references_dict[a]['xml_uri'])[0] + '[\\/\\\\].*\\.xmi'
                # print(local_pattern)

                # print('target:',annotators_dict[b][c],end='\r')
                if len(re.findall(local_pattern,annotators_dict[b][c])) > 0:
                    local_index = c
                    break
            # inner_content[annotators_dict[b]] = annotators_dict[b][c]
            # inner_content[b] = annotators_dict[b][c]
            # print('found local_index:',local_index)
            inner_content[b] = annotators_dict[b][local_index]
        trace_files.append({
            'index':index,
            'tree':tree,
    ...
    for a in range(len(trace_files)):
        ...
        #MOD
        # print(obj['__tags_list__'][-1],end='\r')
        ...
        for a in obj.keys():
            ...
            if a != '__tags_list__':
                ...
                if local_depth == 0:
                    ...
                    output_uri = 'inception_export_' + str(now.year) + '-' + str(now.month) + '-' + str(now.day) + '_file' + str(out_files_count) + '_' + current_annotator + '.xml'
                    a = 0
                    while os.path.exists(output_uri):
                        ...
                        output_uri = 'inception_export_' + str(now.year) + '-' + str(now.month) + '-' + str(now.day) + '_' + str(a) + '_file' + str(out_files_count) + '_' + current_annotator + '.xml'
                        a += 1
                ...
                # for b in range(len(obj[])):
                    ...
                    # output_str += obj[a][b]['inner_content'][annotators_count]
                    # output_str += obj[a][b]['inner_content'][annotators_dict.keys()[annotators_count]]
                    try:
                        # output_str += obj[a][b]['inner_content'][current_annotator]
                        output_str += convert_from_uima(obj[a][b]['inner_content'][current_annotator])
                    except:
                        output_str += 'DEFAULT_INNER_CONTENT_MULT_RECONV'
    ...
    # for a in annotators_dict.keys():
    for a_local in annotators_dict.keys():
        current_annotator = a_local
        # print('Generating corpus for',a,end='\r')
        # print('Generating corpus for',a)
        print('Generating corpus for',a_local)
        output_str = recursive_xml_building('',document_structure,local_depth)
        annotators_count += 1
        # current_annotator = a_local
